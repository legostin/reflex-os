...
- `integration.mcpStatus({ provider?, serverName?, includeConfig? })` ->
  inspect provider MCP configuration across accessible targets, including the
  app-owned project.
...
- `storage.delete({ key })` or `storage.delete({ keys })` -> `{ ok, deleted, missing }`.
  ...
  browser storage is only a compatibility fallback inside app iframes.
