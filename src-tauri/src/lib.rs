...
- Доступные методы bridge:\n\
  ...
  • project.files.list/read/search/write/mkdir/delete — файлы linked project; search может искать UTF-8 content с лимитами; write/mkdir/delete всегда требуют permission project.files.write:<project>|*; .reflex всегда закрыт\n\
...
- Overlay helpers: reflexInvoke, reflexBridgeCatalog, reflexSystemContext, reflexSystemOpenUrl/OpenPath/RevealPath, reflexLog/LogList, reflexManifestGet/Update, reflexPermissions*, reflexNetwork*, reflexWidgets*, reflexActions*, reflexCapabilities, reflexAgentAsk/StartTopic/Task/Stream/StreamAbort, reflexStorageGet/Set/List/Delete, reflexFsRead/List/Write/Delete, reflexClipboardReadText/WriteText, reflexNetFetch, reflexDialogOpenDirectory/OpenFile/SaveFile, reflexNotifyShow, reflexProjectsList/Open, reflexTopicsList/Open, reflexSkillsList, reflexMcpServers, reflexProjectFilesList/Read/Search/Write/Mkdir/Delete, reflexBrowser*, reflexMemory*, reflexScheduler*, reflexAppsList, reflexAppsOpen, reflexAppsInvoke, reflexAppsListActions, reflexEventOn/Off/Emit.\n\
...
fn build_app_creation_prompt(
    ...
    p.push_str("  window.reflexProjectsList(params), reflexProjectsOpen(projectIdOrParams), reflexTopicsList(params), reflexTopicsOpen(threadIdOrParams, projectId?), reflexSkillsList(params), reflexMcpServers(params), reflexProjectFilesList(pathOrParams, recursive?), reflexProjectFilesRead(pathOrParams), reflexProjectFilesSearch(queryOrParams, includeContent?), reflexProjectFilesWrite(pathOrParams, content?), reflexProjectFilesMkdir(pathOrParams), reflexProjectFilesDelete(pathOrParams, recursive?)\n");
