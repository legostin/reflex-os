...
import { useI18n, type LanguageSetting, type Translate } from "../../i18n";
import "./settings.css";
...
type BridgeRecipeCard = (typeof BRIDGE_RECIPE_CARDS)[number];
...
function catalogTitle(title: string, t: Translate): string {
  const key = BRIDGE_GROUP_TITLE_KEYS[title];
  return key ? t(key) : title;
}

function recipeTitle(recipe: BridgeRecipeCard, t: Translate): string {
  const key = RECIPE_TEXT_KEYS[recipe.title]?.title;
  return key ? t(key) : recipe.title;
}

function recipeBody(recipe: BridgeRecipeCard, t: Translate): string {
          ...
          <div className="settings-api-grid">
            {visibleApiGroups.map((group) => (
              <article className="settings-api-group" key={group.title}>
                <h3>{catalogTitle(group.title, t)}</h3>
          ...
          <div className="settings-recipe-grid">
            {visibleRecipeCards.map((recipe) => (
              <article className="settings-recipe-card" key={recipe.title}>
                <h3>{recipeTitle(recipe, t)}</h3>
                <p>{recipeBody(recipe, t)}</p>
                <div className="settings-method-list">
