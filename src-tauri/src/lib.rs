...
- Не используй dialog.*, clipboard.*, system.openUrl/openPath/revealPath, apps.create/delete/restore/purge, apps.open, projects.open, topics.open и scheduler.runNow/setPaused/upsert/delete внутри schedule.steps.\n\
...
fn build_app_creation_prompt(
    ...
    p.push_str("- В steps НЕЛЬЗЯ использовать dialog.openDirectory/openFile/saveFile, clipboard.readText/writeText, system.openUrl/openPath/revealPath, apps.create/delete/restore/purge, apps.open, projects.open и topics.open — у автоматизаций нет UI.\n");
    ...
    p.push_str("  window.reflexAppsList(params), reflexAppsCreate(descriptionOrParams, template?), reflexAppsDelete(appIdOrParams), reflexAppsTrashList(), reflexAppsRestore(trashIdOrParams), reflexAppsPurge(trashIdOrParams), reflexAppsOpen(appIdOrParams), reflexAppsInvoke(appId, actionId, params), reflexAppsListActions(appIdOrParams, includeSteps?)\n");
    ...
    p.push_str("  [\"apps.manage\"]                         — delete/restore/purge/list trash для installed apps\n");
    ...
    p.push_str("- В schedule.steps нельзя использовать dialog.*, clipboard.*, system.openUrl/openPath/revealPath, apps.create/delete/restore/purge, apps.open, projects.open и topics.open: эти шаги бегут без UI.\n\n");
