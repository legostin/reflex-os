...
- Доступные методы bridge:\n\
  ...
  • manifest.get() / manifest.update({{patch}}); widgets.list/upsert/delete — безопасно читать/обновлять manifest и dashboard widgets\n\
...
fn build_app_creation_prompt(description: &str, template: &str) -> String {
    ...
    p.push_str("  window.reflexManifestGet(), reflexManifestUpdate(patch), reflexWidgetsList(), reflexWidgetsUpsert(widgetOrParams), reflexWidgetsDelete(widgetIdOrParams, deleteEntry?), reflexCapabilities() // manifest summary + hasPermission()/hasNetworkHost()\n");
