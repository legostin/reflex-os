...
- Доступные методы bridge:\n\
  ...
  • project.files.list/read — read-only UTF-8 файлы linked project; чужие проекты требуют permission project.files.read:<project>|*; .reflex всегда закрыт\n\
...
- Overlay helpers: reflexInvoke, reflexBridgeCatalog, reflexSystemContext, reflexSystemOpenUrl/OpenPath/RevealPath, reflexLog/LogList, reflexManifestGet/Update, reflexPermissions*, reflexNetwork*, reflexWidgets*, reflexActions*, reflexCapabilities, reflexAgentAsk/StartTopic/Task/Stream/StreamAbort, reflexStorageGet/Set/List/Delete, reflexFsRead/List/Write/Delete, reflexClipboardReadText/WriteText, reflexNetFetch, reflexDialogOpenDirectory/OpenFile/SaveFile, reflexNotifyShow, reflexProjectsList/Open, reflexTopicsList/Open, reflexSkillsList, reflexMcpServers, reflexProjectFilesList/Read, reflexBrowser*, reflexMemory*, reflexScheduler*, reflexAppsList, reflexAppsOpen, reflexAppsInvoke, reflexAppsListActions, reflexEventOn/Off/Emit.\n\
...
fn build_app_creation_prompt(
    ...
    p.push_str("  window.reflexProjectsList(params), reflexProjectsOpen(projectIdOrParams), reflexTopicsList(params), reflexTopicsOpen(threadIdOrParams, projectId?), reflexSkillsList(params), reflexMcpServers(params), reflexProjectFilesList(pathOrParams, recursive?), reflexProjectFilesRead(pathOrParams)\n");
