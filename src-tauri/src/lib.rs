...
- Доступные методы bridge:\n\
  ...
  • manifest.get() / manifest.update({{patch}}); widgets.list/upsert/delete; actions.list/upsert/delete — безопасно обновлять manifest, widgets и callable actions\n\
...
- Overlay helpers: reflexInvoke, reflexBridgeCatalog, reflexSystemContext, reflexSystemOpenUrl/OpenPath/RevealPath, reflexLog/LogList, reflexManifestGet/Update, reflexWidgets*, reflexActions*, reflexCapabilities, reflexAgentAsk/StartTopic/Task/Stream/StreamAbort, reflexStorageGet/Set/List/Delete, reflexFsRead/List/Write/Delete, reflexClipboardReadText/WriteText, reflexNetFetch, reflexDialogOpenDirectory/OpenFile/SaveFile, reflexNotifyShow, reflexProjectsList/Open, reflexTopicsList/Open, reflexSkillsList, reflexMcpServers, reflexBrowser*, reflexMemory*, reflexScheduler*, reflexAppsList, reflexAppsOpen, reflexAppsInvoke, reflexAppsListActions, reflexEventOn/Off/Emit.\n\
...
fn build_app_creation_prompt(
    ...
    p.push_str("  window.reflexManifestGet(), reflexManifestUpdate(patch), reflexWidgetsList(), reflexWidgetsUpsert(widgetOrParams), reflexWidgetsDelete(widgetIdOrParams, deleteEntry?), reflexActionsList(), reflexActionsUpsert(actionOrParams), reflexActionsDelete(actionIdOrParams), reflexCapabilities() // manifest summary + hasPermission()/hasNetworkHost()\n");
    ...
    p.push_str("  window.reflexProjectsList(params), reflexProjectsOpen(projectIdOrParams), reflexTopicsList(params), reflexTopicsOpen(threadIdOrParams, projectId?), reflexSkillsList(params), reflexMcpServers(params)\n");
