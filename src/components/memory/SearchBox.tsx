  ...
  return (
    <div className="memory-search">
      <div className="memory-search-row">
        ...
        >
          {loading ? t("memory.search.loading") : t("memory.search.button")}
        </button>
      </div>
      ...
      <ul className="memory-search-results">
        {hits.length === 0 && !loading && query && !error && (
          <li className="memory-empty">{t("memory.search.noResults")}</li>
        )}
        {hits.map((hit, i) => (
          <li key={`${hit.doc_id}:${i}`} className="recall-list-item">
            <div className="recall-rag-meta">
              <span>{hit.kind}</span>
              <span>
                {t("memory.search.score", { score: hit.score.toFixed(3) })}
              </span>
