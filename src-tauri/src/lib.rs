...
- Доступные методы bridge:\n\
  ...
  • browser.init / project.browser.setEnabled / tabs.list/open/navigate/readText/readOutline/screenshot/clickText/clickSelector/fill — встроенный browser sidecar; project Browser MCP toggle требует mcp.write:<project>|*\n\
...
- В schedule.steps используй bridge methods без UI: agent.task, storage.*, fs.*, project.files.*, net.fetch, notify.show, events.*, apps.invoke, memory.*, manifest.*, scheduler.list/runs/runDetail.\n\
- Не используй dialog.*, clipboard.*, system.openUrl/openPath/revealPath, apps.create/import/delete/restore/purge, apps.open, projects.open, topics.open и scheduler.runNow/setPaused/upsert/delete внутри schedule.steps.\n\
...
fn build_app_creation_prompt(
    ...
    p.push_str("  browser.init({headless?, projectId?}); project.browser.setEnabled({projectId?, enabled}) -> {ok, enabled, server_names}; browser.tabs.list(); browser.open({url?}); browser.navigate({tabId, url})\n");
    ...
    p.push_str("- Требует manifest.permissions: \"browser.read\" для чтения или \"browser.control\" для init/open/navigate/click/fill. Project browser state требует linked project или \"browser.project:<project>\"; включение Reflex Browser MCP через project.browser.setEnabled требует \"mcp.write:<project>\" или \"mcp.write:*\".\n\n");
    ...
    p.push_str("- В steps НЕЛЬЗЯ использовать dialog.openDirectory/openFile/saveFile, clipboard.readText/writeText, system.openUrl/openPath/revealPath, apps.create/import/delete/restore/purge, apps.open, projects.open и topics.open — у автоматизаций нет UI.\n");
    p.push_str("- Все остальные методы (agent.*, storage.*, fs.*, project.files.*, net.fetch, notify.show, events.*, apps.invoke, memory.*, manifest.*, scheduler.list/runs/runDetail) работают как обычно. scheduler.runNow/setPaused/upsert/delete в schedule.steps заблокированы, чтобы не запускать рекурсивные unattended-циклы.\n");
    ...
    p.push_str("  window.reflexManifestGet(), reflexManifestUpdate(patch), reflexPermissionsList(), reflexPermissionsEnsure(permissionOrParams), reflexPermissionsRevoke(permissionOrParams), reflexNetworkHosts(), reflexNetworkAllowHost(hostOrParams), reflexNetworkRevokeHost(hostOrParams), reflexWidgetsList(), reflexWidgetsUpsert(widgetOrParams), reflexWidgetsDelete(widgetIdOrParams, deleteEntry?), reflexActionsList(), reflexActionsUpsert(actionOrParams), reflexActionsDelete(actionIdOrParams), reflexCapabilities() // manifest summary + hasPermission()/hasNetworkHost()\n");
    ...
    p.push_str("  window.reflexBrowserInit(params), reflexProjectBrowserSetEnabled(projectIdOrParams, enabled?), reflexBrowserTabs(), reflexBrowserOpen(url), reflexBrowserNavigate(tabId, url)\n");
    ...
    p.push_str("  window.reflexMemorySearch(queryOrParams), reflexMemoryRecall(queryOrParams), reflexMemoryReindex(params)\n");
    ...
    p.push_str("  window.reflexAppsList(params), reflexAppsCreate(descriptionOrParams, template?), reflexAppsExport(appIdOrParams, targetPath?), reflexAppsImport(zipPathOrParams), reflexAppsDelete(appIdOrParams), reflexAppsTrashList(), reflexAppsRestore(trashIdOrParams), reflexAppsPurge(trashIdOrParams), reflexAppsServerStatus(appIdOrParams), reflexAppsServerLogs(appIdOrParams), reflexAppsServerStart(appIdOrParams), reflexAppsServerStop(appIdOrParams), reflexAppsServerRestart(appIdOrParams), reflexAppsOpen(appIdOrParams), reflexAppsInvoke(appId, actionId, params), reflexAppsListActions(appIdOrParams, includeSteps?)\n");
    ...
    p.push_str("  [\"apps.manage\"]                         — export/import/delete/restore/purge/list trash и server runtime ops для installed apps\n");
    ...
    p.push_str("- В schedule.steps нельзя использовать dialog.*, clipboard.*, system.openUrl/openPath/revealPath, apps.create/import/delete/restore/purge, apps.open, projects.open и topics.open: эти шаги бегут без UI. apps.export допустим для backup-автоматизаций с явным targetPath.\n\n");
